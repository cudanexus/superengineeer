...
export function createMockGitService(): jest.Mocked<GitService> {
  return {
    ...
    commitPaths: jest.fn().mockResolvedValue({ hash: 'abc123', message: 'Test commit' } as CommitResult),
    createBranch: jest.fn().mockResolvedValue(undefined),
    checkout: jest.fn().mockResolvedValue(undefined),
    push: jest.fn().mockResolvedValue('Pushed successfully'),
    pull: jest.fn().mockResolvedValue('Pulled successfully'),
    mergeToMain: jest.fn().mockResolvedValue({ sourceBranch: 'rewind/test', targetBranch: 'main', pushed: true }),
    deleteBranch: jest.fn().mockResolvedValue(undefined),
    promoteCurrentHeadToBranch: jest.fn().mockResolvedValue('abc123def456'),
