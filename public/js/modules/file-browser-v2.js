/**
 * File Browser Module V2 - With Memory Leak Fixes
...
(function(root, factory) {
  ...
  // Debounced functions
  var debouncedSyncScroll = null;
  var debouncedUpdateHighlighting = null;
  ...
  function init(deps) {
    ...
    MemoryCleanup = deps.MemoryCleanup || window.MemoryCleanup;

    // Initialize cleanup manager
    if (MemoryCleanup) {
      cleanupManager = MemoryCleanup.createCleanupManager('FileBrowser');

      // Create debounced functions
      debouncedSyncScroll = MemoryCleanup.debounceWithCleanup(syncEditorScroll, 16, cleanupManager);
      debouncedUpdateHighlighting = MemoryCleanup.debounceWithCleanup(updateEditorSyntaxHighlighting, 100, cleanupManager);
      debouncedSearch = MemoryCleanup.debounceWithCleanup(performSearch, 300, cleanupManager);
    }
  ...
  function renderFileTree(basePath, entries) {
    ...
    // Clean up existing tree before rendering new one
    cleanupTreeElements($tree);
    $tree.empty();
  ...
  function cleanupTreeElements($container) {
    // Remove all event listeners from tree items before clearing
    if (MemoryCleanup) {
      $container.find('.file-tree-item').each(function() {
        MemoryCleanup.removeAllListeners(this);
      });
      $container.find('.btn-delete-file').each(function() {
        MemoryCleanup.removeAllListeners(this);
      });
    }
  }
  ...
  function toggleDirectory(dirPath) {
    if (state.fileBrowser.expandedDirs[dirPath]) {
      ...
      // Clean up children before removing
      if ($children.length) {
        cleanupTreeElements($children);
        $children.remove();
      }
    } else {
  ...
  function openFile(filePath, fileName) {
    ...
    // Limit number of open files to prevent memory issues
    if (state.openFiles.length >= 20) {
      showToast('Too many files open. Please close some files first.', 'warning');
      return;
    }
  ...
  function setActiveFile(filePath) {
    ...
    if (file) {
      ...
      if (file.isMarkdown && file.previewMode) {
        ...
        if (debouncedUpdateHighlighting) {
          debouncedUpdateHighlighting(filePath, file.content);
        } else {
  ...
  function renderOpenFileTabs() {
    ...
    // Clean up existing tabs before rendering new ones
    $tabs.find('.file-tab').each(function() {
      if (MemoryCleanup) {
        MemoryCleanup.removeAllListeners(this);
      }
    });
  ...
  function renderSearchResults(results, searchTerm) {
    ...
    $results.find('.file-search-result').each(function() {
      if (MemoryCleanup) {
        MemoryCleanup.removeAllListeners(this);
      }
    });
    ...
    var displayText = results.length > 100 ? '100+ results (showing first 100)' : results.length + ' result' + (results.length > 1 ? 's' : '');
    $results.html('<div class="text-gray-400 text-xs px-2 py-1 border-b border-gray-700">Found ' + displayText + '</div>');
  ...
  function performSearch() {
    var searchTerm = $('#file-search-input').val().trim();
  ...
  function updateTreeSelection() {
    ...
    if (state.fileBrowser.selectedFile) {
      $('.file-tree-item[data-path="' + CSS.escape(state.fileBrowser.selectedFile) + '"]').addClass('selected');
    }
  }

  function setupHandlers() {
    // Use memory-tracked event listeners if available
    var addEventListener = cleanupManager ? cleanupManager.addEventListener.bind(cleanupManager) : $(document).on.bind($(document));

    // Click on file tree item (but not on delete button)
    addEventListener(document, 'click', function(e) {
      var $target = $(e.target);
      var $item = $target.closest('.file-tree-item');

      if (!$item.length || $target.closest('.btn-delete-file').length) {
        return;
      }
    ...
    addEventListener(document, 'click', function(e) {
      var $target = $(e.target);
      var $btn = $target.closest('.btn-delete-file');

      if (!$btn.length) return;

      e.stopPropagation();
      var path = $btn.data('path');
      var isDir = $btn.data('is-dir') === true || $btn.data('is-dir') === 'true';
    ...
    addEventListener(document, 'contextmenu', function(e) {
      ...
      // Use native event listener for one-time handler
      setTimeout(function() {
    ...
    addEventListener(document, 'click', function(e) {
      var $target = $(e.target);
      var $tab = $target.closest('.file-tab');

      if (!$tab.length || $target.closest('.tab-close').length || $target.closest('.tab-preview-toggle').length) {
        return;
      }
    ...
    addEventListener(document, 'click', function(e) {
      var $target = $(e.target);
      var $btn = $target.closest('.tab-close');
    ...
    $('#file-editor-textarea').on('input', function() {
      ...
      // Use debounced version if available
      if (debouncedUpdateHighlighting) {
        debouncedUpdateHighlighting(state.activeFilePath, currentContent);
      } else {
    ...
    $('#btn-refresh-files').on('click', function() {
      ...
      if (project && project.path) {
        // Clear expanded dirs but maintain selection
    ...
    addEventListener(document, 'click', function(e) {
      var $target = $(e.target);
      var $result = $target.closest('.file-search-result');

      if (!$result.length) return;

      var filePath = $result.data('path');
      var isDir = $result.data('is-dir') === true;
  ...
  function setupDragAndDrop() {
    ...
    // Drag start
    addEventListener(document, 'dragstart', function(e) {
      var $target = $(e.target);
      if (!$target.hasClass('file-tree-item')) return;

      draggedElement = e.target;
      draggedPath = $target.attr('data-path');
    ...
    addEventListener(document, 'dragover', function(e) {
      var $target = $(e.target);
      var $item = $target.closest('.file-tree-item.directory');
      ...
      var targetPath = $item.attr('data-path');
    ...
    // Drag end
    addEventListener(document, 'dragend', function(e) {
      var $target = $(e.target);
      if (!$target.hasClass('file-tree-item')) return;
  ...
  function initializeResize() {
    ...
    var addEventListener = cleanupManager ? cleanupManager.addEventListener.bind(cleanupManager) : function(el, ev, fn) { $(el).on(ev, fn); };

    // Handle mouse down on resize handle
    addEventListener($resizeHandle[0], 'mousedown', function(e) {
      isResizing = true;
      startX = e.clientX;
    ...
    addEventListener($collapseBtn[0], 'click', function() {
    ...
    addEventListener($resizeHandle[0], 'touchstart', function(e) {
    ...
    addEventListener(document, 'touchend', function() {
      if (touchStartX === 0) return;
    ...
    addEventListener(document, 'touchcancel', function() {
      if (touchStartX === 0) return;
